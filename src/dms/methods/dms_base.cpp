...
DmsBase::DmsBase(const t_state* state, const t_mdatoms* tmdatoms,
		...
		std::string cgType, bool deb, char* topFname) : freqUpdate(freq), timeStep(0), dim(aDim), cgDim(cDim), assembleFreq(assFreq), debug(deb) {
	...
	registerMethods();
		...
		fpLog << getTime() << ":INFO:Initializing constructor" << std::endl;
    		...
    		if ( fineGrainHash.find(cgMethod) == fineGrainHash.end() ) {
           	...
           	Microscopic = new Micro_state(state, tmdatoms, top, ir, dim, comm, fineGrainHash[cgMethod], mSteps, dt, this, topFname); // why cantI pass fpLog (fsream) here?!!!!!!
		...
		ierr = initializeHash[cgMethod](*this);
            	...
            	Mesoscopic  = new Meso_state(nCG, cgDim, nHist, comm, coarseGrainHash[cgMethod]);
...
DmsBase::DmsBase(const t_state* state, const t_mdatoms* tmdatoms,
		...
		const gmx_int64_t freq, const real dt, const gmx_int64_t t0, MPI_Comm communic, const int mSteps, const double optimScale, const int nss, 
		const int nssGlob, const int nHist, std::string cgType, char* userRef, char* topFname) { 
	...
	if(!mpiRank) {
		...
		if ( fineGrainHash.find(cgMethod) == fineGrainHash.end() ) {
               	...
               	Microscopic = new Micro_state(state, tmdatoms, top, ir, dim, comm, fineGrainHash[cgMethod], mSteps, dt, this, topFname);
               	nAtoms = Microscopic->Get_DOF() / numSSglob;
...
int DmsBase::cgStep(gmx_int64_t gromacStep) {
	...
	if(!mpiRank) {
		...
		*/
		...
		if(timeStep == 1)
			for(int dim = 0; dim < Mesoscopic->Get_Dim(); dim++) {
                        	ierr = VecCopy(Mesoscopic->Get_RefCoords()[dim], Mesoscopic->Get_Coords()[dim]);
                        	CHKERRQ(ierr);
                	}
		...
		*/
		...
		if(tprRecord->nstxout > 0) {
			if(gromacStep % tprRecord->nstxout == 0)
				writePetsc(Mesoscopic->Get_Coords(), keys, timeStep, &viewer);
		}
		else {
			if(tprRecord->nstxout_compressed > 0)
				if(gromacStep % tprRecord->nstxout_compressed == 0)
					writePetsc(Mesoscopic->Get_Coords(), keys, timeStep, &viewer);
		}
	}
...
dmsBasePtr newDmsBase(const t_state* state, const t_mdatoms* mdatoms,
		      ...
		      const int nss, const int nssGlob, const int nHist, char const* cgMeth, char* userRef, 
		      char* topFname) {
    ...
    return reinterpret_cast<void*>(new DmsBase(state, mdatoms, top, ir, dim, cgDim, nCG, freq, dt, t0, comm, mSteps, 
				   scale, nss, nssGlob, nHist, cgMethod, userRef, topFname));
}
...
PetscErrorCode DmsBase::computeVelocs() {
        /* This function constructs SWM coords, velocities, and forces, depending on the arguments
         * supplied. The three CG-construction function pointers in Mesoscopic should all point to
         * this function.
         */
        ...
        for(int dim = 0; dim < Mesoscopic->Get_Dim(); dim++) {
                ...
                ierr = (Mesoscopic->mappingVelo)(Microscopic->Get_Velocities()[dim], timeStep, dim, *this);
                CHKERRQ(ierr);
        }
...
PetscErrorCode DmsBase::constructVelocities() {
	...
	fpLog << getTime() << ":INFO:Constructing CG velocities " << std::endl;
