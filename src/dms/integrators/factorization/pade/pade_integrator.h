 ...
 * new integrator should publicly inherit from DmsIntegrator and override the method Integrate().
 *
...
class DmsPade : public DmsIntegrator {
	PetscScalar delta;
	PetscInt nHist;
	Mat invMat;
public:
	explicit DmsPade(PetscScalar dt, PetscScalar dtMicro, bool Adpt = false, MPI_Comm comm = PETSC_COMM_SELF);
	PetscErrorCode computeInvMat(PetscScalar Coords[]);
	virtual ~DmsPade() {};
	virtual PetscErrorCode integrate(const std::vector< CVec >& Coords, const CVec& RHS);	
};
