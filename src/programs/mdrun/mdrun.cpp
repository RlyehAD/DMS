...
#define NFILE asize(fnm)
	...
	{ "-dms_relax", FALSE, etINT, {&dmsRelax},
          "force DMS to relax the system for a numbero of MD steps (after fine-graining)" },
	{ "-dms_freq", FALSE, etINT64, {&dmsFreq},
	  "number of CG steps covered before updating the reference configuration" },
	{ "-dms_dt",   FALSE, etINT, {&dmsDt},
	  "CG time step (fs)" },
        { "-dms_micro",   FALSE, etINT, {&dmsMicro},
          "length of the micro (MD) phase (fs)" },
	{ "-dms_hist",   FALSE, etINT, {&dmsHist},
          "number of previous time steps to use for the multiscale time integrator" },
	{ "-dms_elecAmp",   FALSE, etREAL, {&elecFieldAmp},
          "amplitude (kJ/mol*nm*eC) of the electric field applied along the z-axis" },
	{ "-dms_elecFreq",   FALSE, etREAL, {&elecFieldFreq},
          "frequency (GHz) of the external electric field applied along the z-axis" },
	{ "-dms_elecFieldPhase",   FALSE, etREAL, {&elecFieldFreq},
          "phase of the electric field applied along the z-axis" },
        { "-dms_elecMultiFreq",   FALSE, etSTR, {&dmsElecMulti},
	...
	int nHarm;
    	...
    	FILE *fp = fopen(dmsElecMulti, "r");
	fscanf(fp, "%d", &dArgs.nElecHarmonics);
    	
	dArgs.elecFieldFreq  = (real*) malloc(dArgs.nElecHarmonics * sizeof(real));
	dArgs.elecFieldAmp   = (real*) malloc(dArgs.nElecHarmonics * sizeof(real)); 
	dArgs.elecFieldPhase = (real*) malloc(dArgs.nElecHarmonics * sizeof(real));

	for(nHarm = 0; nHarm < dArgs.nElecHarmonics; nHarm++)
    		fscanf(fp, "%f %f %f", dArgs.elecFieldFreq + nHarm, dArgs.elecFieldAmp + nHarm, dArgs.elecFieldPhase + nHarm);

	fclose(fp);
    }
    else
	dArgs.nElecHarmonics = 0;
    ...
    rc = mdrunner(&hw_opt, fplog, cr, NFILE, fnm, oenv, bVerbose, bCompact,
                  ...
                  pforce, cpt_period, max_hours, deviceOptions, imdport, Flags, &dArgs);
