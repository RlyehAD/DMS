...
real dmsInstantElecField(real t, real *elecFieldFreq, real *elecFieldAmp, real *elecFieldPhase, int nElecHarmonics) {	//Y
/*
elecFieldFreq [GHz]
elecFieldPhase [fraction of a period for a given frequency]
*/
	int i;	// harmonic counter
	real elecPeriod;	// [ns] Period for a given electric field harmonics
	real t_rel;		// [ns] Time since the beginning of last period of the elec field oscillation, assuming init. phase 0
	real elecField = 0.;		// Electric field
	for ( i=0; i<nElecHarmonics; i++ )
	{
		elecPeriod = 1.0 / elecFieldFreq[i];
	        t_rel = fmod(t / 1000.0, elecPeriod);
	        elecField += elecFieldAmp[i] * cos(2.0 * M_PI * (elecFieldFreq[i] * t_rel + elecFieldPhase[i]));
	}
        return elecField;
}
	     ...
	     dmsArgs* dArgs)
{
    ...
    int kmax = dArgs->kmax, dimCG = 3, dmsRelax = dArgs->relax;
    gmx_int64_t numFreq = dArgs->freq;
    real dtDms = dArgs->dt / 1000.0;
    const int microSteps = dArgs->micro;
    int nFrames = microSteps; // must be read from tpr

    float dmsScale = dArgs->scale;
	...
	if(dArgs->nElecHarmonics > 0) {
		...
		for (i=0; i < dArgs->nElecHarmonics; i++)	//Y
			printf("%f\t%f\t%f\n", dArgs->elecFieldAmp[i], dArgs->elecFieldFreq[i], dArgs->elecFieldPhase[i]);	//Y
	}
    }
   ...
   if(MASTER(cr)) {
    ...
    while (!bLastStep || (bRerunMD && bNotLastFrame))
    {
	    ...
	    // apply external electric field
            real elecField = dmsInstantElecField(t, dArgs->elecFieldFreq, dArgs->elecFieldAmp, dArgs->elecFieldPhase, dArgs->nElecHarmonics);	//Y
    ...
    /* free memory allocated for elecField pointers */
    sfree(dArgs->elecFieldFreq); sfree(dArgs->elecFieldAmp); sfree(dArgs->elecFieldPhase);	//Y
