...
void dmsDistributeCoords(gmx_domdec_t *dd, rvec *x, rvec *x_local) {

	dd_distribute_vec(dd, dd_charge_groups_global(dd), x, x_local);
}


void dmsDistributeState(FILE                *fplog,
                        gmx_int64_t          step,
			gmx_domdec_t *dd, t_state *state, t_state *state_local,
			t_commrec *cr,
                        t_inputrec *ir) {
	...
	gmx_ddbox_t        ddbox = {0};        
	set_ddbox(dd, TRUE, cr, ir, state->box,
                  TRUE, cgs_gl, state->x, &ddbox);

        get_cg_distribution(fplog, step, dd, cgs_gl, state->box, &ddbox, state->x);

	dd_distribute_vec(dd, cgs_gl, state->x, state_local->x);
}
