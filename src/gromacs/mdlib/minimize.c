                            ...
                            gmx_int64_t count, gmx_bool bFirst)
{
    ...
    do_force(fplog, cr, inputrec,
             ...
             (bNS ? GMX_FORCE_NS | GMX_FORCE_DO_LR : 0), 0);
...
double do_steep2(FILE *fplog, t_commrec *cr,
                int nfile, const t_filenm fnm[],
                const output_env_t gmx_unused oenv, gmx_bool bVerbose, gmx_bool gmx_unused bCompact,
                int gmx_unused nstglobalcomm,
                gmx_vsite_t *vsite, gmx_constr_t constr,
                ...
                dmsArgs* dArgs)
{
    ...
    {
        /* Print to the screen  */
        sp_header(stderr, SD, ir->em_tol, nsteps);
    }
    if (fplog)
    {
        sp_header(fplog, SD, ir->em_tol, nsteps);
    }

    do_em_step(cr, ir, mdatoms, fr->bMolPBC,
               s_min, stepsize, s_min->f, s_try,
               constr, top, nrnb, wcycle, 0);
}
